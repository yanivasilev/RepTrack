...
exports.countPushups = countPushups;
 ...
 */
function pickBestSide(kps, minScore) {
...
function elbowAngleFromFrame(kps, side, minScore) {
    ...
    if (!s || !e || !w)
        return null;
    if (s.score < minScore || e.score < minScore || w.score < minScore)
        return null;
 ...
 */
function hipPlankAngleFromFrame(kps, side, minScore) {
    ...
    if (!sh || !hp || !an)
        return null;
    if (sh.score < minScore || hp.score < minScore || an.score < minScore)
        return null;
    return angleDeg(sh, hp, an);
}
function shoulderAndHipY(kps, side, minScore) {
    const sh = getPoint(kps, side === "left" ? "left_shoulder" : "right_shoulder");
    const hp = getPoint(kps, side === "left" ? "left_hip" : "right_hip");
    if (!sh || !hp)
        return null;
    if (sh.score < minScore || hp.score < minScore)
        return null;
    return { shoulderY: sh.y, hipY: hp.y, shoulder: sh, hip: hp };
}
function shoulderYFromFrame(kps, side, minScore) {
    const sh = getPoint(kps, side === "left" ? "left_shoulder" : "right_shoulder");
    if (!sh || sh.score < minScore)
        return null;
    return sh.y;
}
// smoothing helper
...
function gradeRep(m) {
    ...
    const SLOW = 6.5;
    if (m.lowConfidence) {
        return { code: "LOW_CONFIDENCE", message: "Tracking was shaky—try better lighting and keep full body in frame." };
    }
    if (m.travelFrac < MIN_TRAVEL_FRAC) {
        return { code: "NO_TRAVEL", message: "Make the motion bigger—lower your chest more." };
    }
    if (m.duration < FAST) {
        return { code: "TOO_FAST", message: "Slow down—control the rep." };
    }
    if (m.duration > SLOW) {
        return { code: "TOO_SLOW", message: "A bit quicker—keep a steady tempo." };
    }
    if (m.minPlankAngle !== 999 && m.minPlankAngle < PLANK_GOOD) {
        return { code: "BAD_PLANK", message: "Keep a straighter line—avoid hips hiking/sagging." };
    }
    if (m.minElbowAngle !== 999 && m.minElbowAngle > DEPTH_GOOD) {
        return { code: "TOO_SHALLOW", message: "Go lower for full depth." };
    }
    return { code: "GOOD", message: "Good rep!" };
}
/**
 ...
 */
function countPushups(frames, opts) {
    // -----------------------
    // TUNABLE PARAMETERS
    // -----------------------
    ...
    let minPlankAngle = null;
    let elbowSmoothed = null;
    let confidenceFrames = 0;
    let missingFrames = 0;
    let repSignal = "SHOULDER_Y";
    const resetRep = () => {
        repStartT = null;
        topShoulderY = null;
    ...
    for (const f of frames) {
        ...
        if (yInfo) {
            ...
            if (tl > 1)
                torsoLen = torsoLen == null ? tl : torsoLen * 0.9 + tl * 0.1;
        }
        ...
        if (plankAng != null)
            minPlankAngle = minPlankAngle == null ? plankAng : Math.min(minPlankAngle, plankAng);
        if (elbowAng != null)
            minElbowAngle = minElbowAngle == null ? elbowAng : Math.min(minElbowAngle, elbowAng);
    ...
    return {
        ...
        usedFrames,
        isLikelyPushup: true,
        feedback: repFeedbacks.length === 0
