...
type Fields = keyof ForgotPasswordResetType;

type ForgotPasswordResetResponse =
    | { success: true; message: string }
    | { success: false; message: string; errors?: Partial<Record<Fields, string>> };

export async function SubmitForgotPasswordReset({ data }: { data: ForgotPasswordResetType }): Promise<ForgotPasswordResetResponse> {
    const result = await submitForm<ForgotPasswordResetType, Fields, ApiSucess>({
        data,
        validate: (d) => {
